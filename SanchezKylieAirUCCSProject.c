...
int getValideInt(int min, int max, int sentinel){
    int numNights=0;
    puts("Enter the number of nights you want to rent the property");
    int scanned = scanf("%d", &numNights);
    
    while((numNights<min && numNights!=sentinel ) || numNights>max || scanned==0){
        if(numNights==0) {
...
int main (void){
    ...
    while(rentalMode){
        ...
        int numNights = getValideInt(MIN_RENTAL_NIGHTS, MAX_RENTAL_NIGHTS, SENTINAL_NEG1);
        if (numNights==SENTINAL_NEG1){
            rentalMode=false;
        }
        else{
            int charge = calculateCharges(numNights, INTERVAL_1_NIGHTS, INTERVAL_2_NIGHTS, RENTAL_RATE, DISCOUNT);
            numNights = totalNights+numNights;
            totalNights=totalCharges+charge;
